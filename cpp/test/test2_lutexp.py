...
def test_lut(lut_address_size, lut_input_range_upscale, input_range, exp_out_scale, exp_in_scale=None):
    ...
    f.subplots_adjust(bottom=0.05, top=0.95, hspace=0.125, left=0.15,
                      right=0.95)
    ...
    ax2.plot(x, err_diff_float_prop * 100, 'x',
             label='Proportional error (double) in %')
    ...
    za_yrange = (-10, 10)
    zb_yrange = (100, 350)
    ax1a.set_xlim(za_xrange)
    ax1a.set_ylim(za_yrange)
    ax1b.set_xlim(zb_xrange)
    ax1b.set_ylim(zb_yrange)
