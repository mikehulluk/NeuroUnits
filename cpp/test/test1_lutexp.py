...
def test_lut(lut_address_size, lut_input_range_upscale, input_range, exp_out_scale, exp_in_scale=None,):
    ...
    exp_in_scale = (exp_in_scale if exp_in_scale
                    is not None else lut_input_range_upscale)

    x = np.linspace(input_range[0], input_range[1], num=1000)
    exp_x_int = getexp(from_float(x, exp_in_scale), exp_in_scale,
                       exp_out_scale)
    exp_x = to_float(exp_x_int, exp_out_scale)

    err_diff_float_prop = np.fabs(np.exp(x) - exp_x) / np.exp(x)
    err_diff_int = np.fabs(from_float(np.exp(x), exp_out_scale)
                           - exp_x_int)
...
test_lut(lut_address_size=5, lut_input_range_upscale=3,
         input_range=(-6, 6), exp_out_scale=10)
test_lut(lut_address_size=5, lut_input_range_upscale=3,
         input_range=(-6, -2), exp_out_scale=-2)
#
test_lut(lut_address_size=7, lut_input_range_upscale=3,
         input_range=(-6, 6), exp_out_scale=10)
test_lut(lut_address_size=7, lut_input_range_upscale=3,
         input_range=(-6, -2), exp_out_scale=-2)
#
test_lut(lut_address_size=7, lut_input_range_upscale=4,
         input_range=(-15, -14), exp_out_scale=-19)

test_lut(lut_address_size=10, lut_input_range_upscale=3,
         input_range=(-6, 6), exp_out_scale=10)
test_lut(lut_address_size=10, lut_input_range_upscale=3,
         input_range=(-6, -2), exp_out_scale=-2)
#
